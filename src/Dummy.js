( ( _root ) => {
    ...
    Dummy = {
        ...
        dummy() {
            ...
            for( let i = 0; i < dummyTextFillTags.length; i++ ) {
                ...
                switch ( dummyConfig.length ) {
                    ...
                    case 3:
                        var configArray = dummyConfig[ 2 ].split( '-' )
                          , config = {};

                        config.min = parseInt( configArray[ 0 ] );
                        config.max = parseInt( configArray[ 1 ] );
                    ...
                    case 4:
                        var firstConfigArray = dummyConfig[ 2 ].split( '-' )
                          , firstConfig = {};

                        var secondConfigArray = dummyConfig[ 3 ].split( '-' )
                          , secondConfig = {};

                        firstConfig.min = parseInt( firstConfigArray[ 0 ] );
                        firstConfig.max = parseInt( firstConfigArray[ 1 ] );

                        secondConfig.min = parseInt( secondConfigArray[ 0 ] );
                        secondConfig.max = parseInt( secondConfigArray[ 1 ] );

                        currentElement.innerText = this.generateLoremBlock( dummyConfig[ 1 ], dummyConfig[ 0 ], firstConfig, secondConfig );
                        break;

                    default:
                        this.log( 'Invalid number of arguments supplied' );
        ...
        generateLoremBlock( type, count, firstConfig, secondConfig ) {
            ...
            switch( type ) {
                ...
                case 'sentence':
                case 'sentences':
                    return this.loremSentences( ipsumArray, count, firstConfig );

                case 'paragraph':
                case 'paragraphs':
                    return this.loremParagraphs( ipsumArray, count, firstConfig, secondConfig );
        ...
        loremSentences( dict, number, config ) {
            ...
            for( let i = 0; i < number; i++ ) {
                ...
                sentenceLength = ( config == null ) ? this.randomInt( 10, 13 ) : this.randomInt( config.min, config.max );
        ...
        loremParagraphs( dict, number, paragraphConfig, sentenceConfig ) {
            ...
            for( let i = 0; i < number; i++ ) {
                ...
                paragraphLength = ( paragraphConfig == null ) ? this.randomInt( 10, 13 ) : this.randomInt( paragraphConfig.min, paragraphConfig.max );

                ipsumString += this.loremSentences( dict, paragraphLength, sentenceConfig ).trim() + '\r\n\r\n';
